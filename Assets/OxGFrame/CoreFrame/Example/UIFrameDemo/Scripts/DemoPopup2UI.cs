...
public class DemoPopup2UI : UIBase
{
    ...
    private async void _ShowDemoPopup3UI()
    {
        if (this.uiSetting.canvasType == CanvasType.CanvasCamera)
            await UIManager.GetInstance().Show(UIFrameDemo.screenId, UIFrameDemo.ScreenDemo3UI, null, UIFrameDemo.ScreenDemoLoadingUI, null);
        else if (this.uiSetting.canvasType == CanvasType.CanvasWorld)
            await UIManager.GetInstance().Show(UIFrameDemo.worldId, UIFrameDemo.WorldDemo3UI, null, UIFrameDemo.WorldDemoLoadingUI, null);

        // Bundle
        //await UIManager.GetInstance().Show("ui", "Demo3UI");
    }
}
