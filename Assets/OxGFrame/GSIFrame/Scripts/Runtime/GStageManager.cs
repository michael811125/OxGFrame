...
{
    public class GStageManager<T> where T : GStageManager<T>, new()
    {
        ...
        public byte currentGstId { get; private set; }               // 紀錄當前的 GameStageId
        public GStage currentGStage { get; private set; }            // 當前 GameStage
        ...
        public void ChangeGameStageForce(byte gstId)
        {
            ...
            this.currentGstId = this._incomingGstId = gstId;    // 立即指定curGstId & incomingGstId = gstId
        ...
        protected void UpdateGameStage(float dt = 0.0f)
        {
            ...
            if (this.currentGstId != this._incomingGstId)
            {
                ...
                this.currentGstId = this._incomingGstId;    // 將incomingGstId記錄至curGstId
                this.InitGameStage();                   // 開始進行GameStage初始的處理過程
            }

            if (this.currentGStage == null) return;

            if (this.currentGStage.runUpdate) this.currentGStage.UpdateStage(dt); // 開始刷新當前的GameStage
        }
        ...
        protected void InitGameStage()
        {
            ...
            this.currentGStage = this.GetGameStage(this.currentGstId);   // 透過當前的GameStageId取出GameStage, 並且指定至當前GameStage中
            this.currentGStage.StartInitStage().Forget();            // 開始進行GameStage初始流程
        }
